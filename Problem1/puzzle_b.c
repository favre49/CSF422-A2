 ...
 *
 * p_array is an array of pointers to priority queues of size k.
 */
hash_table* h = NULL;
priority_queue** p_array = NULL;
...
void* parallelThread(void* arg)
{
    ...
    while(1)
    {
        ...
        while(is_empty(p_array[sidx]));

        current_board = top(p_array[sidx]);
...
int main(int argc, char** argv)
{
    ...
    h = create_hash_table();
    p_array = (priority_queue**)malloc(sizeof(priority_queue*)*k);
    for (int i = 0; i < k; i++)
        p_array[i] = create_priority_queue();

    // Insert first board in.
    insert(h,initial_board);
    push(p_array[0],initial_board);
